...
# Import csv file, transform to wide data, call it dat
dat <- read_csv(paste(datpath_clean, "/bugrdat.csv", sep=""))
dat2<-dat %>% mutate(cover=cover+0.00001)
...
spp.mds0  #by default 2 dimensions returned, stress is 6.41
ordiplot(spp.mds0)
...
help(metaMDS)
spp.mds<-metaMDS(cover.relrow, trace = TRUE, autotransform=F, trymax=100, k=4) #runs several with different starting configurations
